  ...
  return (
    <BookmarkContextMenu bookmark={bookmark} onEdit={onEdit} onDelete={onDelete}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <Card className="overflow-hidden hover:shadow-md transition-all duration-300 border-transparent hover:border-primary/20">
          <CardContent className="p-0">
            <div className="flex items-center justify-between gap-3 p-3 hover:bg-accent/50 transition-all duration-300 group">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="flex flex-col min-w-0 space-y-1">
                  <span className="text-sm font-medium truncate group-hover:text-primary transition-colors duration-200">{bookmark.title}</span>
                  <span className="text-xs text-muted-foreground/70 truncate group-hover:text-muted-foreground transition-colors duration-200">{bookmark.url}</span>
                </div>
              </div>
